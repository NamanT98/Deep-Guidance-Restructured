...
CHASEDB1DataSetPath = "D:\DIP\DeepGuidance\data\CHASEDB1"
ORIGADatasetPath = "D:\\DIP\\DeepGuidance\\data\\ORIGA"
ORIGAMaskPath = "D:\\DIP\\DeepGuidance\\data\\ORIGA\\Semi-automatic-annotations"
REFUGEDATASETPATH = ""
    ...
    for i in range(batch_size):
        if dataset == "DRIVE":
            img_path = os.path.join(
                DRIVEDataSetPath, "images", img_name[i].rstrip("\n")
            )
            if flag == "train":
        ...
        if dataset == "STARE":
            img_path = os.path.join(
                STAREDataSetPath, "images", img_name[i].rstrip("\n")
            )
            # label_name = img_name[i].rstrip('\n')[:-4] + '.vk.ppm'
            label_name = img_name[i].rstrip("\n")[:-4] + ".ah.ppm"
        ...
        if dataset == "CHASEDB1":
            img_path = os.path.join(
                CHASEDB1DataSetPath, "images", img_name[i].rstrip("\n")
            )
            label_name = img_name[i].rstrip("\n")[:-4] + "_1stHO.png"
        ...
        if dataset == "ORIGA":
            img_path = os.path.join(
                ORIGADatasetPath, "images", img_name[i].rstrip("\n")
            )
            label_name = img_name[i].rstrip("\n")[:-4] + ".mat"
            label_path = os.path.join(ORIGAMaskPath, label_name)
            mask_name = img_name[i].rstrip("\n")[:-4] + ".png"
            img = cv2.imread(img_path)
            label = scipy.io.loadmat(label_path)["mask"]
            # label = cv2.imread(label_path)
            ...
            mask_path = os.path.join(ORIGADatasetPath, "Masks", mask_name)
            FOVmask = cv2.imread(mask_path)
            FOVmask = FOVmask[:, :, 0]
...
def get_img_list(dataset, SubID, flag="train"):
    ...
    if dataset == "CHASEDB1":
        if flag == "train":
            with open(
                os.path.join(CHASEDB1DataSetPath, "CHASEDB1training.txt"), "r"
            ) as f:
                img_list = f.readlines()
        else:
            with open(
                os.path.join(CHASEDB1DataSetPath, "CHASEDB1testing.txt"), "r"
            ) as f:
                img_list = f.readlines()

    if dataset == "ORIGA":
        if flag == "train":
            with open(os.path.join(ORIGADatasetPath, "TrainOriga.txt"), "r") as f:
                img_list = f.readlines()
        else:
            with open(os.path.join(ORIGADatasetPath, "TestOriga.txt"), "r") as f:
                img_list = f.readlines()
