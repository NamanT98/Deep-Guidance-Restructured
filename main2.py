...
import argparse
import time
from core.utils import get_img_list, get_model, get_data
from pylab import *
import random
from test2 import fast_test
...
dataset_list = ["DRIVE", "STARE", "CHASEDB1", "ORIGA"]

# --------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="PyTorch")

parser.add_argument("--epochs", type=int, default=100, help="the epochs of this run")
parser.add_argument(
    "--n_class",
    type=int,
    default=3,
...
if not os.path.exists(
    r"%s\\models\\%s_%s" % (RootDir, model_name, args.my_description)
):
    os.mkdir(
        r"%s\DeepGuidance\models\%s_%s" % (RootDir, model_name, args.my_description)
    )

with open(
    r"%s\\logs\\%s_%s.txt" % (RootDir, model_name, args.my_description), "w+"
) as f:
...
for epoch in range(args.epochs):
    ...
    for i, (start, end) in enumerate(
        ...
        e_optic_disk = calculate_voe(ppi, gt.cpu().numpy().squeeze(), 1)
        e_optic_cup = calculate_voe(ppi, gt.cpu().numpy().squeeze(), 2)

        print(
            str(
                "model: {:s}_{:s} | epoch_batch: {:d}_{:d} | loss: {:f}  | E_optic_cup: {:f} | E_optic_disk: {:f}"
            ).format(
                model_name,
                args.my_description,
                epoch,
    ...
    if epoch % 5 == 0 and epoch != 0:
        ...
        e_disc, e_cup = fast_test(
            model,
            args,
            test_img_list,
            model_name,
            epoch,
            f"D:\\DIP\\DeepGuidance\\results\\{Dataset}\\train",
        )
        # print("Best E_DISK)
        if e_disc < best_e_disc or e_cup < best_e_cup:
            best_e_disc = min(e_disc, best_e_disc)
            best_e_cup = min(e_cup, best_e_cup)
            torch.save(
                model.state_dict(),
                "%s\\models\\%s_%s\\%s.pth"
                % (RootDir, model_name, args.my_description, str(epoch)),
            )
            # For evaluation
